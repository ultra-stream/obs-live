...
codesign_bundle() {
    ...
    if [ ! -d ./OBS-Live.app ]; then
        error "No OBS-Live.app bundle found"
        return
    fi

    hr "Code-signing application bundle"

    xattr -crs ./OBS-Live.app

    read_codesign_ident

    step "Code-sign OBS-Live code..."
    echo -n "${COLOR_ORANGE}"
    codesign --force --options runtime --entitlements "${CI_SCRIPTS}/app/entitlements.plist" --sign "${CODESIGN_IDENT}" --deep ./OBS-Live.app
    echo -n "${COLOR_RESET}"
    step "Check code-sign result..."
    codesign -dvv ./OBS-Live.app
}
...
notarize_macos() {
    ...
    hr "Notarizing OBS-Live for macOS"
    ...
    xcnotary precheck "./OBS-Live.app"

    if [ "$?" -eq 0 ]; then
        read_codesign_ident
        read_codesign_pass

        step "Run xcnotary with ./OBS-Live.app..."
        #step "xcnotary notarize ./OBS-Live.app --developer-account ${CODESIGN_IDENT_USER} --developer-password-keychain-item OBS-Codesign-Password --provider ${CODESIGN_IDENT_SHORT}"
        xcnotary notarize "./OBS-Live.app" --developer-account "${CODESIGN_IDENT_USER}" --developer-password-keychain-item "OBS-Codesign-Password" --provider "Y38V5NVUPS"
    fi
}
