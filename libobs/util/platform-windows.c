...
void get_win_ver(struct win_version_info *info)
{
	...
	if (!got_version) {
		...
		if (ver.major == 10) {
			...
			TCHAR build_value[1024];
			DWORD build_value_length = sizeof(build_value);
			...
			status = RegQueryValueExW(key, L"CurrentBuild", NULL, NULL,
						(LPBYTE)&build_value,
						&build_value_length);
			if (status == ERROR_SUCCESS) {
				int buildNumber = _wtoi(build_value);
