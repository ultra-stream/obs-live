...
static bool selected_window_changed(obs_properties_t *props, obs_property_t *p,
				obs_data_t *settings)
{
	UNUSED_PARAMETER(p);

	const char *windowTitle = obs_data_get_string(settings, "window");
	struct window_capture *wc = bzalloc(sizeof(struct window_capture));
	bfree(wc->title);
	bfree(wc->class);
	bfree(wc->executable);

	build_window_strings(windowTitle, &wc->class, &wc->title, &wc->executable);

	obs_property_t *help_p = obs_properties_get(props, "help");

	struct dstr str = {0};
	dstr_printf(&str, "Get help to configure %s for streaming", wc->executable);

	obs_property_set_description(help_p, str.array);

	return true;
}

static bool GetHelpClicked(obs_properties_t *list, obs_property_t *p, void *data)
{
	system("start https://github.com/ultra-stream/obs-live");
	return true;
}
...
static obs_properties_t *wc_properties(void *unused)
{
	...
	obs_property_set_modified_callback(p, selected_window_changed);

	obs_properties_add_button(ppts, "help", "Get Help",
				GetHelpClicked);
