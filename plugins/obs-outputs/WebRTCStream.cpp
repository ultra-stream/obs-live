...
namespace {
static int gVideoSourceCount = 0;
...
WebRTCStream::WebRTCStream(obs_output_t *output)
{
    ...
    info("++ WebRTCStream::WebRTCStream");
...
bool WebRTCStream::start(WebRTCStream::Type type)
{
    ...
    if (!isServiceValid) {
        obs_output_set_last_error(
            output,
            "Failed to establish streaming connection. Please check your network.");
        obs_output_signal_stop(output, OBS_OUTPUT_CONNECT_FAILED);
        return false;
    }
    ...
    gVideoSourceCount = CountVideoSources();
    if (gVideoSourceCount) {
...
bool WebRTCStream::close(bool wait)
{
    ...
    if (client) {
        ...
        // Commenting out client destruction to avoid random crash on stop of streaming SL3-25
        //delete(client);
        client = nullptr;
    }
    return true;
