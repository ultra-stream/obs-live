...
void OBSBasic::InitDefaultTransitions()
{
	...
	for (OBSSource &tr : transitions) {
		m_transitions->addItem(QT_UTF8(obs_source_get_name(tr)),
...
void OBSBasic::CreateDefaultQuickTransitions()
{
	...
	quickTransitions.emplace_back(GetTransitionComboItem(m_transitions,
							     0),
...
obs_source_t *OBSBasic::FindTransition(const char *name)
{
	for (int i = 0; i < m_transitions->count(); i++) {
		OBSSource tr = m_transitions->itemData(i).value<OBSSource>();
...
void OBSBasic::SetTransition(OBSSource transition)
{
	...
	if (oldTransition && transition) {
		...
		if (transition != GetCurrentTransition())
			SetComboTransition(m_transitions, transition);
		obs_set_output_source(0, transition);
		obs_transition_swap_end(transition, oldTransition);
	} else {
		obs_set_output_source(0, transition);
...
{
	return m_transitions->currentData().value<OBSSource>();
}
...
void OBSBasic::AddTransition()
{
	...
	if (accepted) {
        ...
        m_transitions->addItem(
			QT_UTF8(name.c_str()),
			QVariant::fromValue(OBSSource(source)));
        m_transitions->setCurrentIndex(m_transitions->count() - 1);
...
void OBSBasic::QuickTransitionChange()
{
	...
	if (qt) {
		qt->source = GetTransitionComboItem(m_transitions, trIdx);
		ResetQuickTransitionText(qt);
	}
}
