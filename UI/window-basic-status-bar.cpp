...
OBSBasicStatusBar::OBSBasicStatusBar(QWidget *parent)
	: QWidget(parent),
	  ...
	  redPixmap(20, 20)
{
	streamTime->setText(QString("LIVE STREAMING TIME: 00:00:00"));
    ...
    mainLayout->addWidget(droppedFrames);
    mainLayout->addWidget(streamTime);
    mainLayout->addWidget(recordTime);
    mainLayout->addWidget(cpuUsage);
    mainLayout->addWidget(delayInfo);
    mainLayout->addWidget(brWidget);
...
{
	OBSBasic *main = qobject_cast<OBSBasic *>(MainWindow());
	if (!main)
		return;
...
void OBSBasicStatusBar::UpdateStreamTime()
{
	...
	QString text;
	text.sprintf("LIVE STREAMING TIME: %02d:%02d:%02d", hours, minutes, seconds);
...
void OBSBasicStatusBar::SetMainWindow(OBSBasic *main)
{
    mainWindow = main;
}
OBSBasic *OBSBasicStatusBar::MainWindow()
{
    return mainWindow;
}
