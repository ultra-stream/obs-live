...
void OBSBasicSettings::LoadStream1Settings()
{
	...
	if (strcmp(type, "rtmp_custom") == 0) {
		...
		const char *username =
			obs_data_get_string(settings, "sourceLiveUsername");
		const char *password =
			obs_data_get_string(settings, "sourceLivePassword");
		const char *passkey =
			obs_data_get_string(settings, "sourceLivePasskey");
		ui->authUsername->setText(QT_UTF8(username));
		ui->authPw->setText(QT_UTF8(password));
		ui->authPasskey->setText(QT_UTF8(passkey));
...
void OBSBasicSettings::SaveStream1Settings()
{
	...
	if (!customServer && webrtc == 0) {
		...
		if (ui->useAuth->isChecked()) {
			obs_data_set_string(
				settings, "sourceLiveUsername",
				QT_TO_UTF8(ui->authUsername->text()));
			obs_data_set_string(settings, "sourceLivePassword",
				QT_TO_UTF8(ui->authPw->text()));
			obs_data_set_string(settings, "sourceLivePasskey",
				QT_TO_UTF8(ui->authPasskey->text()));
		}
	} else if (webrtc > 0) {
		obs_data_set_string(settings, "server",
				QT_TO_UTF8(ui->customServer->text()));
		...
		obs_data_set_string(settings, "sourceLiveUsername",
				QT_TO_UTF8(ui->authUsername->text()));
		obs_data_set_string(settings, "sourceLivePassword",
				QT_TO_UTF8(ui->authPw->text()));
 		obs_data_set_string(settings, "sourceLivePasskey",
...
void OBSBasicSettings::LoadServices(bool showAll)
{
    ...
    for (std::vector<std::string>::size_type i = webrtc_count; i-- > 0; )
    {
        std::string tmp = obs_service_get_display_name(webrtc_services[i].c_str());
        if (tmp == std::string("Obs-Live"))
            ui->service->insertItem(
            0, obs_service_get_display_name(webrtc_services[i].c_str()),
            QVariant((int)i+3));
    }
