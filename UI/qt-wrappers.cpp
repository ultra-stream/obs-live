...
void OBSMessageBox::critical(QWidget *parent, const QString &title,
			     const QString &text, QString helpButtonURL)
{
	QMessageBox mb(QMessageBox::Critical, title, text, QMessageBox::Ok | QMessageBox::Help,
		       parent);
	mb.setButtonText(QMessageBox::Ok, QTStr("OK"));

	if (!helpButtonURL.isEmpty()) {
		mb.setButtonText(QMessageBox::Help, QTStr("Help"));
	}
    QMessageBox::StandardButton button = (QMessageBox::StandardButton)mb.exec();
    if (!helpButtonURL.isEmpty() && button == QMessageBox::Help) {
        QDesktopServices::openUrl(helpButtonURL);
    }
}
