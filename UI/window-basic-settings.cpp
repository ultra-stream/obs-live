...
static void AddSettingsComboItem(QComboBox *combo, obs_property_t *prop,
...
#define SLCREDS_CHANGED SLOT(SLCredsChanged())
	  ...
	  ui(new Ui::OBSBasicSettings)
{
	...
	HookWidget(ui->authPasskey,          EDIT_CHANGED,   STREAM1_CHANGED);
	HookWidget(ui->codec,                COMBO_CHANGED,  STREAM1_CHANGED);
	HookWidget(ui->streamProtocol,       COMBO_CHANGED,  STREAM1_CHANGED);
        HookWidget(ui->authUsername,         EDIT_CHANGED,   SLCREDS_CHANGED);
        HookWidget(ui->authPw,               EDIT_CHANGED,   SLCREDS_CHANGED);
        HookWidget(ui->authPasskey,          EDIT_CHANGED,   SLCREDS_CHANGED);
	...
	HookWidget(ui->recordingFilePathLineEdit, EDIT_CHANGED, OUTPUTS_CHANGED);
	HookWidget(ui->advOutNoSpace,        CHECK_CHANGED,  OUTPUTS_CHANGED);
	HookWidget(ui->openRecordedFileLocationCheckBox, CHECK_CHANGED, OUTPUTS_CHANGED);
...
#undef PROCESS_PRIORITY
	...
	connect(ui->channelSetup, SIGNAL(currentIndexChanged(int)), this,
		SLOT(ChannelChangedWarning(int)));
	connect(ui->channelSetup, SIGNAL(currentIndexChanged(int)), this,
	...
	ui->buttonBox->button(QDialogButtonBox::Save)->setIcon(QIcon());
...
#ifdef WIN32
    ui->openRecordedFileLocationCheckBox->setStyleSheet("QCheckBox {font-size:15px;}");
#endif

    connect(ui->advancedAudioSettingsButton, &QPushButton::clicked, this, &OBSBasicSettings::on_audio_advanced_settins_clicked);
    connect(ui->passkeyShowButton, &QPushButton::clicked, this, &OBSBasicSettings::on_show_passkey_button_clicked);
    connect(ui->rematrixButton, &QPushButton::clicked, this, &OBSBasicSettings::on_rematrix_button_clicked);
    connect(ui->changeRecordingPathButton, &QPushButton::clicked, this, &OBSBasicSettings::on_recordingFilePathLineEdit_clicked);

    if (!main->volumes.empty()) {
        int idx = -1;

        obs_source_t *source = obs_get_source_by_name("Micro");
        if (source) {
            ...
            for (size_t i = 0; i < count; i++)
                AddSettingsComboItem(ui->audioSourceDeviceComboBox, prop, format, i);

            string value = from_obs_data(settings, name, format);

            if (idx == -1) {
                idx = ui->audioSourceDeviceComboBox->findData(QByteArray(value.c_str()));
            }

            ui->audioSourceDeviceComboBox->setMaxVisibleItems(40);

            if (idx != -1)
                ui->audioSourceDeviceComboBox->setCurrentIndex(idx);
            ...
            blog(LOG_INFO, "Can't resolve Micro source on Settings dialog creation.");
        }
    }

    if (main->StreamingActive() || main->StreamingStarting()) {
        ui->outputResolution->setEnabled(false);
    }
}

void OBSBasicSettings::on_audio_advanced_settins_clicked()
{
...
void OBSBasicSettings::on_show_passkey_button_clicked()
{
    QLineEdit::EchoMode echoMode = ui->authPasskey->echoMode();
    if (echoMode == QLineEdit::Password) {
        ui->authPasskey->setEchoMode(QLineEdit::Normal);
        ui->passkeyShowButton->setText("HIDE KEY");
    } else {
        ui->authPasskey->setEchoMode(QLineEdit::Password);
        ui->passkeyShowButton->setText("SHOW KEY");
    }
}

void OBSBasicSettings::on_rematrix_button_clicked()
{
    obs_source_t *source = obs_get_source_by_name("Micro");
    if (!source) {
        source = main->currentActiveVolControl->GetSource();
    }
    RematrixDialog *rematrixDialog = new RematrixDialog(this, source, main);
    rematrixDialog->exec();
    rematrixDialog->deleteLater();
}

void OBSBasicSettings::on_advAudioProps_destroyed()
{
...
{
	uint32_t cx = config_get_uint(App()->GlobalConfig(), "Video", "BaseCX");
	uint32_t cy = config_get_uint(App()->GlobalConfig(), "Video", "BaseCY");
	uint32_t out_cx = config_get_uint(App()->GlobalConfig(), "Video", "OutputCX");
	uint32_t out_cy = config_get_uint(App()->GlobalConfig(), "Video", "OutputCY");
...
void OBSBasicSettings::LoadAdvOutputRecordingSettings()
{
    ...
    bool openRecordedFileLocation = config_get_bool(main->Config(), "AdvOut",
                                   "OpenRecordedFileLocation");

	int typeIndex = (astrcmpi(type, "FFmpeg") == 0) ? 1 : 0;
	ui->advOutRecType->setCurrentIndex(typeIndex);
	ui->advOutRecPath->setText(path);
	ui->recordingFilePathLineEdit->setText(path);
	ui->advOutNoSpace->setChecked(noSpace);
	ui->openRecordedFileLocationCheckBox->setChecked(openRecordedFileLocation);
...
void OBSBasicSettings::SaveVideoSettings()
{
	...
	// Hardcode to FPS.Common (0 value)
    int fpsType = 0;
	uint32_t cx = 0, cy = 0;
	...
	if (WidgetChanged(ui->baseResolution) &&
		...
		config_set_uint(App()->GlobalConfig(), "Video", "BaseCX", cx);
		config_set_uint(App()->GlobalConfig(), "Video", "BaseCY", cy);
		config_save_safe(App()->GlobalConfig(), "tmp", nullptr);
	}

	if (WidgetChanged(ui->outputResolution) &&
	    ConvertResText(QT_TO_UTF8(outputResolution), cx, cy)) {
		config_set_uint(App()->GlobalConfig(), "Video", "OutputCX", cx);
		config_set_uint(App()->GlobalConfig(), "Video", "OutputCY", cy);
		config_save_safe(App()->GlobalConfig(), "tmp", nullptr);
	}
...
void OBSBasicSettings::SaveOutputSettings()
{
    ...
    SaveEdit(ui->recordingFilePathLineEdit, "AdvOut", "RecFilePath");
	SaveCheckBox(ui->advOutNoSpace, "AdvOut", "RecFileNameWithoutSpace");
	SaveCheckBox(ui->openRecordedFileLocationCheckBox, "AdvOut", "OpenRecordedFileLocation");
...
void OBSBasicSettings::SaveSettings()
{
    ...
    if (source) {
        ...
        if (type == OBS_COMBO_TYPE_EDITABLE) {
            data = ui->audioSourceDeviceComboBox->currentText().toUtf8();
        } else {
            int index = ui->audioSourceDeviceComboBox->currentIndex();
            if (index != -1)
                data = ui->audioSourceDeviceComboBox->itemData(index);
            else
                return;
        }
...
void OBSBasicSettings::on_recordingFilePathLineEdit_clicked()
{
    QString dir = QFileDialog::getExistingDirectory(
            this, "Select Recording",
            ui->recordingFilePathLineEdit->text(),
            QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (dir.isEmpty())
        return;

    ui->recordingFilePathLineEdit->setText(dir);
}
...
void OBSBasicSettings::SLCredsChanged()
{
        if (!loading) {
                slCredsChanged = true;
        }
...
void OBSBasicSettings::ChannelChangedWarning(int idx)
{
	...
	//if (surround && !wasSurround) {
		...
		QString warningString = "You will need to relaunch Obs-Live to allow changes to take an effect.\nDo you wish to continue?";
