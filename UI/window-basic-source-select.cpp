	    ...
	    const bool visible, OBSSource &newSource)
{
	...
	if (source) {
		...
		if (source) {
#ifdef WIN32
			const char* id = obs_obj_get_id(source);
			if (strcmp(id, "monitor_capture") == 0) {
				obs_data_t *settings = obs_data_create();
				obs_source_save_into_settings(source, settings);
				bool initialized = obs_data_get_bool(settings, "initialized");
				if (!initialized) {
					...
					if (ver.major == 10) {
						...
						if (res == QMessageBox::Open) {
							QDesktopServices::openUrl(
								QUrl("https://github.com/ultra-stream/obs-live"));
						}
					}
				}
			}
