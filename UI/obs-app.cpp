...
static void do_log(int log_level, const char *msg, va_list args, void *param)
{
	...
	if (log_level <= LOG_DEBUG || log_verbose) {
...
bool OBSApp::InitTheme()
{
	...
	// Always overwrite CurrentTheme with OBSLive theme
	themeName = "OBSLive";
...
{
#ifdef OBS_LIVE_PRODUCTION_BUILD
    return;
#endif
	stringstream dst;
	...
	try {
		...
		if (!multi) {
			...
			QMessageBox mb(QMessageBox::Question,
				       "OBS-Live is already launched.",
				       "OBS-Live is already launched. Do you want to continue and run multiple instances at the same time?", buttons,
				       nullptr);
...
#define CRASH_MESSAGE "OBS-Live crashed. Please try to restart the app."
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	int ret = MessageBoxA(NULL, CRASH_MESSAGE, "OBS-Live crashed.",
