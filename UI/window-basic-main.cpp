	...
	: OBSMainWindow(parent), ui(new Ui::OBSBasic)
{
	...
	ui->setupUi(this);
        ...
        QObject::connect(this->networkManager,
                         ...
                         this, &OBSBasic::userStatusReplyReady);
    ui->statusbar->SetMainWindow(this);
    ui->settingsLabel->setText("SETTINGS");
	...
	connect(ui->selectSourceButton, SIGNAL(pressed()), this, SLOT(selectSourceButtonPressed()));
	connect(ui->clearSourcesButton, SIGNAL(pressed()), this, SLOT(clearSourcesButtonPressed()));
	connect(ui->visitGatewayButton, SIGNAL(pressed()), this, SLOT(visitGatewayButtonPressed()));

	connect(ui->actionAbout, SIGNAL(triggered()), this, SLOT(on_action_about_triggered()));

	bool recordWhenStreaming = config_get_bool(
		GetGlobalConfig(), "BasicWindow", "RecordWhenStreaming");
	ui->recordingEnabledCheckbox->setChecked(recordWhenStreaming);

	connect(ui->recordingEnabledCheckbox, SIGNAL(stateChanged(int)), this, SLOT(on_recording_checkbox_state_changed(int)));

	userBalancePollTimer = new QTimer(this);
	userBalancePollTimer->setInterval(1000 * 60 * 2);
	connect(userBalancePollTimer, &QTimer::timeout, this, &OBSBasic::pollSEUserStatusText);
        QMetaObject::invokeMethod(this, "pollSEUserStatusText", Qt::QueuedConnection);
        userBalancePollTimer->start();
}

void OBSBasic::pollSEUserStatusText()
{
        QString username;
        QString password;
        const QString SERVER_URL = QString("%1%2").arg(OBS_LIVE_SERVER_URL).arg("/api/v1/gateway/usage/current_month");
        obs_data_t *settings = obs_service_get_settings(service);

        QString obsLiveUsername = obs_data_get_string(settings, "obsLiveUsername");
        QString obsLivePassword = obs_data_get_string(settings, "obsLivePassword");
        QString obsLivePassKey = obs_data_get_string(settings, "obsLivePasskey");

        if (!obsLiveUsername.isEmpty() && !obsLivePassword.isEmpty() && !obsLivePassKey.isEmpty()) {
                QByteArray credentials =
                        QString("%1:%2")
                                ...
                                .toLocal8Bit()
                                .toBase64();
                QString authHeader = "Basic " + credentials;
                QNetworkRequest request =
                        QNetworkRequest(QUrl(SERVER_URL));
                request.setRawHeader("Authorization",
                                     authHeader.toLocal8Bit());
...
void OBSBasic::userStatusReplyReady(QNetworkReply *reply)
{
#ifndef OBS_LIVE_PRODUCTION_BUILD
	++queryCounter;
#endif
        ...
        if (networkError == QNetworkReply::NoError) {
                ...
                if (jsonData.contains(statusKey)) {
                        QString status = jsonData[statusKey].toString();
#ifndef OBS_LIVE_PRODUCTION_BUILD
                        status.append(QString(" (Query Number %1)").arg(queryCounter));
#endif
                        ui->userBalanceStatusLabel->setText(status);
                        //blog(LOG_ERROR, "Received reply %s", QString(replyStr).toStdString().c_str());
                }
        }
        reply->deleteLater();
}

void OBSBasic::selectSourceButtonPressed()
{
    AddSourcePopupMenu(QCursor::pos());
}

void OBSBasic::clearSourcesButtonPressed()
{
    on_actionRemoveSource_triggered();
}

void OBSBasic::visitGatewayButtonPressed()
{
    obs_data_t *settings = obs_service_get_settings(service);
    QString obsLiveUsername = obs_data_get_string(settings, "obsLiveUsername");
    const QString url = "https://github.com/ultra-stream/obs-live/%1";
    const QString gatewayUrl = QString("%1%2").arg(OBS_GATEWAY_URL).arg("/%1");
    QDesktopServices::openUrl(gatewayUrl.arg(obsLiveUsername));
}

void OBSBasic::on_action_about_triggered()
{
    OBSLiveAbout *aboutDlg = new OBSLiveAbout(this);
    aboutDlg->exec();
    aboutDlg->deleteLater();
}

void OBSBasic::on_recording_checkbox_state_changed(int state)
{
    bool recordWhenStreaming = ui->recordingEnabledCheckbox->isChecked();
    config_set_bool(GetGlobalConfig(), "BasicWindow", "RecordWhenStreaming", recordWhenStreaming);
}
...
retryScene:
	...
	currentScene = obs_scene_from_source(curScene);
...
bool OBSBasic::InitBasicConfigDefaults()
{
	 ...
	 * changing */
	if (!config_has_user_value(App()->GlobalConfig(), "Video", "OutputCX") ||
		!config_has_user_value(App()->GlobalConfig(), "Video", "OutputCY")) {
		config_set_uint(App()->GlobalConfig(), "Video", "OutputCX", scale_cx);
		config_set_uint(App()->GlobalConfig(), "Video", "OutputCY", scale_cy);
    ...
    config_set_default_bool(basicConfig, "Video", "AutoRemux", true);
    config_set_bool(basicConfig, "Video", "AutoRemux", true);

    config_set_default_bool(basicConfig, "AdvOut", "OpenRecordedFileLocation", true);
...
void OBSBasic::OBSInit()
{
	...
	onlyAudioLabel = new QLabel(ui->preview);
	onlyAudioLabel->setText("Only Audio");
	QFont font = onlyAudioLabel->font();
	font.setPointSize(20);
	font.setBold(true);
	onlyAudioLabel->setFont(font);
	onlyAudioLabel->setStyleSheet("QLabel { background-color:black; color:red; }");
	onlyAudioLabel->setSizePolicy(ui->preview->sizePolicy());
	onlyAudioLabel->setHidden(true);
...
OBSScene OBSBasic::GetCurrentScene()
{
	if (currentScene == nullptr) {
        currentScene = obs_scene_create("DefaultScene");
	}
	return currentScene;
}
...
VolControl* OBSBasic::currentActiveVolControl = nullptr;
...
void OBSBasic::ActivateAudioSource(OBSSource source)
{
    ...
    currentActiveVolControl = vol;
}

void OBSBasic::DeactivateAudioSource(OBSSource source)
{
	bool deactivated = currentActiveVolControl->GetSource() == source;
	for (size_t i = 0; i < volumes.size(); i++) {
	...
	if (deactivated) {
		currentActiveVolControl = volumes.empty() ? nullptr : volumes[0];
	}
}
...
bool OBSBasic::QueryRemoveSource(obs_source_t *source)
{
	...
	QString text = "Do you want to remove '$1'?";
	text.replace("$1", QT_UTF8(name));

	QMessageBox remove_source(this);
	remove_source.setText(text);
	QAbstractButton *Yes =
		remove_source.addButton("Yes", QMessageBox::YesRole);
	remove_source.addButton("No", QMessageBox::NoRole);
	remove_source.setIcon(QMessageBox::Question);
	remove_source.setWindowTitle("OBS-Live");
	remove_source.exec();
...
int OBSBasic::ResetVideo()
{
	...
	ovi.base_width =
		(uint32_t)config_get_uint(App()->GlobalConfig(), "Video", "BaseCX");
	ovi.base_height =
		(uint32_t)config_get_uint(App()->GlobalConfig(), "Video", "BaseCY");
	ovi.output_width =
		(uint32_t)config_get_uint(App()->GlobalConfig(), "Video", "OutputCX");
	ovi.output_height =
		(uint32_t)config_get_uint(App()->GlobalConfig(), "Video", "OutputCY");
	...
	if (ovi.base_width == 0 || ovi.base_height == 0) {
		...
		config_set_uint(App()->GlobalConfig(), "Video", "BaseCX", 1920);
		config_set_uint(App()->GlobalConfig(), "Video", "BaseCY", 1080);
	}

	if (ovi.output_width == 0 || ovi.output_height == 0) {
		ovi.output_width = ovi.base_width;
		ovi.output_height = ovi.base_height;
		config_set_uint(App()->GlobalConfig(), "Video", "OutputCX",
				ovi.base_width);
		config_set_uint(App()->GlobalConfig(), "Video", "OutputCY",
				ovi.base_height);
		config_save_safe(App()->GlobalConfig(), "tmp", nullptr);
	}
...
void OBSBasic::ResizePreview(uint32_t cx, uint32_t cy)
{
	...
	centerOnlyAudioLabel();
}

void OBSBasic::centerOnlyAudioLabel()
{
	QSize parentSize = ui->preview->geometry().size();
	QSize labelSize = onlyAudioLabel->geometry().size();
	onlyAudioLabel->move(parentSize.width()/2-labelSize.width()/2, parentSize.height()/2-labelSize.height()/2);
}
...
void OBSBasic::closeEvent(QCloseEvent *event)
{
	...
	if (outputHandler && outputHandler->Active()) {
		...
		QMessageBox::StandardButton button = OBSMessageBox::question(
			this, "OBS-Live",
			"There is currently active streaming that will be shut down. Are you sure you wish to exit?");
...
void OBSBasic::on_action_Settings_triggered()
{
	...
	if (settings.CredsChanged()) {
		pollSEUserStatusText();
	}
	SystemTray(false);
...
void OBSBasic::on_actionAddScene_triggered()
{
	...
	if (accepted) {
		...
		obs_source_t *source = obs_get_source_by_name(name.c_str());
		if (source) {
			OBSMessageBox::warning(this, "Name already exists.",
...
void OBSBasic::CreateMouseRightClickSourceMenu(OBSSceneItem item)
{
    ...
    QAction *resizeOutput =
            popup.addAction("Resize View to fit the Source", this,
...
void OBSBasic::AddSource(const char *id)
{
	if (id && *id) {
		...
		if (!OBSBasicSourceSelect::AddNew(this, id, QT_TO_UTF8(text),
                    true, newSource))
            return;
        CreatePropertiesWindow(newSource);
...
QMenu *OBSBasic::CreateAddSourcePopupMenu()
{
	...
	auto addSource = [this, getActionAfter](QMenu *popup, const char *type,
		...
		//blog(LOG_INFO, type);
		//blog(LOG_INFO, "---");
		if (qtype != "dshow_input" && qtype != "window_capture" &&
		    qtype != "ndi_source" && qtype != "monitor_capture" &&
		    qtype != "image_source")
		    return;
		if (qtype == "dshow_input") {
		    qname = "Capture Input Device";
		} else if (qtype == "window_capture") {
			qname = "Capture Window";
		} else if (qtype == "monitor_capture") {
			qname = "Capture Screen";
		} else if (qtype == "image_source") {
			qname = "Capture Image (Watermark)";
		}
#else
		if (qtype != "av_capture_input" && qtype != "window_capture" &&
		    qtype != "ndi_source" && qtype != "display_capture" &&
		    qtype != "image_source")
		    return;
		if (qtype == "av_capture_input") {
		    qname = "Capture Input Device";
		} else if (qtype == "window_capture") {
		    qname = "Capture Window";
		} else if (qtype == "display_capture") {
		    qname = "Capture Screen";
		}  else if (qtype == "image_source") {
			qname = "Capture Image (Watermark)";
		}
...
void OBSBasic::AddSourcePopupMenu(const QPoint &pos)
{
	if (!GetCurrentScene()) {
		...
		OBSMessageBox::information(
			this, "Can't add a source.",
			"At least one scene should exist to add a source.");
		return;
	}
...
static bool remove_all_items(obs_scene_t *, obs_sceneitem_t *item, void *param)
{
...
static bool add_all_items(obs_scene_t *, obs_sceneitem_t *item, void *param)
{
        vector<OBSSceneItem> &items =
                *reinterpret_cast<vector<OBSSceneItem> *>(param);
        items.emplace_back(item);
...
void OBSBasic::on_actionRemoveSource_triggered()
{
	...
	obs_scene_enum_items(GetCurrentScene(), remove_all_items, &items);

	if (!items.size())
		return;

	auto removeMultiple = [this](size_t count) {
		QString text = QString("Do you want to remove %1 items?")
		...
		remove_items.addButton("No", QMessageBox::NoRole);
		remove_items.setIcon(QMessageBox::Question);
		remove_items.setWindowTitle("OBS-Live");
		remove_items.exec();
...
void OBSBasic::removeSource(obs_source_t *source)
{
    const char *sourceName = obs_source_get_name(source);
    OBSSceneItem item = obs_scene_find_source(currentScene, sourceName);
    obs_sceneitem_remove(item);
}
...
void OBSBasic::StartStreaming()
{
	...
	QString username;
	QString password;
	const QString SERVER_URL = QString("%1%2").arg(OBS_LIVE_SERVER_URL).arg("/api/v1/gateway/token");
	const QString nameKey = "stream_name";
	const QString tokenKey = "token";
	obs_data_t *settings = obs_service_get_settings(service);

	QString obsLiveUsername = obs_data_get_string(settings, "obsLiveUsername");
	QStringobsLivePassword = obs_data_get_string(settings, "obsLivePassword");
    QString obsLivePassKey = obs_data_get_string(settings, "obsLivePasskey");

	if (!obsLiveUsername.isEmpty() && !obsLivePassword.isEmpty() && !obsLivePassKey.isEmpty()) {
	QByteArray credentials = QString("%1:%2").arg(obsLiveUsername).arg(obsLivePassword).toLocal8Bit().toBase64();
	QString authHeader = "Basic " + credentials;
	QByteArray postData = QT_TO_UTF8(QString("{ \"passkey\": \"%1\" }").arg(obsLivePassKey));
	QNetworkRequest request = QNetworkRequest(QUrl(SERVER_URL));
	...
	QEventLoop awaitLoop;
	QNetworkAccessManager networkManager;
	QObject::connect(&networkManager, &QNetworkAccessManager::finished, &awaitLoop, &QEventLoop::quit);
	...
	if (networkError == QNetworkReply::NoError) {
	    ...
	    if (jsonData.contains(nameKey) && jsonData.contains(tokenKey)) {
		username = jsonData[nameKey].toString();
		password = jsonData[tokenKey].toString();
	    }
	} else if (networkError == QNetworkReply::AuthenticationRequiredError) {
	    OBSMessageBox::critical(
		    this, "OBS-Live",
		    "This account is not allowed to stream.\nPlease verify entered Username and Password in the Settings.",
		     "https://github.com/ultra-stream/obs-live");
	    ui->streamButton->setChecked(false);
	    return;
	} else if (statusCode == 400) {
	    OBSMessageBox::critical(
		    this, "OBS-Live",
		    "Stream for this user is already active. Please finish active streaming and try again.",
		    "https://github.com/ultra-stream/obs-live");
	    ui->streamButton->setChecked(false);
	    return;
	} else {
    ...
    obs_data_set_string(settings, "username", QT_TO_UTF8(username));
    obs_data_set_string(settings, "password", QT_TO_UTF8(password));
	...
	if (!outputHandler->StartStreaming(service)) {
		...
		QMessageBox::critical(this, "Start Streaming Error",
				      message);
		return;
	}
...
void OBSBasic::StopStreaming()
{
	...
	if (RecordingActive() && !keepRecordingWhenStreamStops)
		StopRecording();
...
{
	ui->streamButton->setText(QTStr("START STREAMING"));
	ui->streamButton->setEnabled(true);
	ui->streamButton->setChecked(false);
...
void OBSBasic::SetStreamingSettingsControlsEnabled(bool enabled)
{
	ui->recordingEnabledCheckbox->setEnabled(enabled);
	if (CountVideoSources() == 0) {
		ui->selectSourceButton->setEnabled(enabled);
		onlyAudioLabel->setHidden(enabled);
		centerOnlyAudioLabel();
	}
}
...
{
	SetStreamingSettingsControlsEnabled(false);
	ui->streamButton->setText("STOP STREAMING");
	ui->streamButton->setEnabled(true);
	ui->streamButton->setChecked(true);
...
{
	ui->streamButton->setText("STOPPING STREAMING");
...
void OBSBasic::StreamingStop(int code, QString last_error)
{
	...
	case OBS_OUTPUT_BAD_PATH:
		errorDescription = "No valid streaming settings were found. Please setup them in the Settings.";
		break;

	case OBS_OUTPUT_CONNECT_FAILED:
		use_last_error = true;
		errorDescription = "Please setup valid settings.";
		break;
	...
	pollSEUserStatusText();
	SetStreamingSettingsControlsEnabled(true);
	ui->streamButton->setText("START STREAMING");
	ui->streamButton->setEnabled(true);
	ui->streamButton->setChecked(false);
	...
	if (encode_error) {
		OBSMessageBox::information(
			this, "Error.",
			"Streaming finished with encoding error.");
...
void OBSBasic::AutoRemux()
{
    ...
    bool openRecordedFileLocation = config_get_bool(Config(), "AdvOut",
                                                    "OpenRecordedFileLocation");
    if (openRecordedFileLocation) {
        revealFileInDefaultFileSystemViewer(output);
    }
}

void OBSBasic::revealFileInDefaultFileSystemViewer(const QString &path)
{
#ifdef _WIN32
...
#define RP_NO_HOTKEY_TITLE "Hotkeys are not set."
#define RP_NO_HOTKEY_TEXT "Set the \"Save\" hotkey to use for saving replay recordings."
...
bool OBSBasic::NoSourcesConfirmation()
{
	if (CountVideoSources() == 0 && isVisible()) {
		...
		msg = "No video streaming sources were selected.\nSelect streaming source or press \"Continue\" to stream audio only.";

		QMessageBox messageBox(this);
		messageBox.setWindowTitle("No video streaming sources were selected.");
...
void OBSBasic::on_streamButton_clicked()
{
    ...
    QString obsLivePassword = obs_data_get_string(settings, "obsLivePassword");
    QString obsLivePassKey = obs_data_get_string(settings, "obsLivePasskey");
    if (obsLiveUsername.isEmpty()) {
        OBSMessageBox::critical(
                this, "OBS-Live",
                "Valid Username must be set in the Settings.");
        ui->streamButton->setChecked(false);
        return;
    }
    if (obsLivePassword.isEmpty()) {
        OBSMessageBox::critical(
                this, "OBS-Live",
                "Valid Password must be set in the Settings.");
        ui->streamButton->setChecked(false);
        return;
    }
    if (obsLivePassKey.isEmpty()) {
        OBSMessageBox::critical(
                this, "OBS-Live",
                "Valid Passkey must be set in the Settings.");
        ui->streamButton->setChecked(false);
        return;
    }

    isStreamingStarting = true;
    startStreaming();
    ...
    if (outputHandler->StreamingActive()) {
        ...
        if (confirm && isVisible()) {
            QMessageBox::StandardButton button =
                    OBSMessageBox::question(
                            this, "OBS-Live",
                            "Are you sure you want to stop the active stream?");
        ...
        if (bwtest && isVisible()) {
            ...
            QMessageBox::StandardButton button =
                    OBSMessageBox::question(
                            this, "OBS-Live",
                            "Do you want to start the stream?");
...
void OBSBasic::GetFPSCommon(uint32_t &num, uint32_t &den) const
{
	...
	if (strcmp(val, "10") == 0) {
		...
		num = 60;
		den = 1;
	} else if (strcmp(val, "23.976") == 0) {
		num = 24000;
		den = 1001;
	} else if (strcmp(val, "24P") == 0) {
		num = 24000;
		den = 1000;
	} else {
		num = 30;
...
void OBSBasic::ResizeOutputSizeOfSource()
{
	...
	OBSSource source = obs_sceneitem_get_source(rightClickedItem);

	int width = obs_source_get_width(source);
	int height = obs_source_get_height(source);

	config_set_uint(App()->GlobalConfig(), "Video", "BaseCX", width);
	config_set_uint(App()->GlobalConfig(), "Video", "BaseCY", height);
	config_set_uint(App()->GlobalConfig(), "Video", "OutputCX", width);
	config_set_uint(App()->GlobalConfig(), "Video", "OutputCY", height);
...
bool OBSBasic::StreamingStarting()
{
    return isStreamingStarting;
}
